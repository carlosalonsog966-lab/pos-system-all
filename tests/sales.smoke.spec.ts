...
test('Sales carga sin errores y muestra contenido', async ({ page }) => {
  const consoleErrors: string[] = [];
  page.on('console', (msg) => {
    if (msg.type() === 'error') consoleErrors.push(msg.text());
  });

  await navigateTo(page, '/#/sales?tm=1');
  await page.waitForLoadState('networkidle');
  await page.waitForFunction(() => document.readyState === 'complete');

  // La ruta puede requerir auth; acepta /sales o redirección a /login
  const current = page.url();
  const okRoute = current.includes('/#/sales') || current.includes('/#/login') || current.includes('/login');
