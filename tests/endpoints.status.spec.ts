...
test.describe('Endpoints Status (exports/status)', () => {
  const baseUrl = 'http://localhost:8080/endpoints.html';
  ...
  test('orden asc/desc invierte primera y última ruta', async ({ page }) => {
    // Desactivar paginación para ver todas las rutas en la misma página
    await page.goto(`${baseUrl}#limit=9999&page=1`, { waitUntil: 'networkidle' });
    const dirSelect = page.locator('#dir');
    await expect(dirSelect).toBeVisible();
